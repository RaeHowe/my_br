...
func NewPdController(
	...
	cli := httputil.NewClient(tlsConf) //http客户端
	...
	for _, addr := range addrs {
		...
		versionBytes, failure = pdRequest(ctx, addr, clusterVersionPrefix, cli, http.MethodGet, nil) //br客户端给pd节点发送http请求，获取集群版本信息
		if failure == nil {
			break
		}
	}
	...
	version := parseVersion(versionBytes) //解析tidb集群版本信息
	...
	return &PdController{
		addrs:    processedAddrs, //各个pd地址: http://tidb-test-pd0
		cli:      cli,            //http客户端
