...
func RunBackup(c context.Context, g glue.Glue, cmdName string, cfg *BackupConfig) error {
	...
	//  <-----------------------------
	err = client.SetLockFile(ctx) //把lock文件：backup.lock 写入到s3路径中，作为互斥锁来使用（加锁）
	if err != nil {
		return errors.Trace(err)
	}
	...
	backupTS, err := client.GetTS(ctx, cfg.TimeAgo, cfg.BackupTS) //获取到备份时间
	if err != nil {
		return errors.Trace(err)
	}
	...
	*/
	...
	isIncrementalBackup := cfg.LastBackupTS > 0 //判断是否是增量备份
